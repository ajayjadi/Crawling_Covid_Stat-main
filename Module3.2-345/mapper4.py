...
def filter_files_by_country(country_name_input, directory_input):
    relevant_files = [filename for filename in os.listdir(directory_input) if country_name_input.lower() in filename.lower()]
    return relevant_files

def extract_year_from_filename(filename_input):
    match = re.search(r'\d{4}', filename_input)
    if match:
        return match.group(0)
    else:
        return None

def merge_files_into_one(output_filename_input, file_list_input, directory_input):
    with open(output_filename_input, 'w') as outfile:
        for filename in file_list_input:
            ...
            if country_year is None:
                ...
                continue
            with open(os.path.join(directory_input, filename), 'r') as infile:
                skip_line = False
                for line in infile:
...
def extract_dates_and_content_from_file(filename_input):
    dates = []
    content = []
...
def main():
    country_name_input = sys.argv[1]
    parent_directory_input = os.path.abspath(os.path.join(os.getcwd(), os.pardir))
    directory_input = os.path.join(parent_directory_input, "Module2")
    relevant_files_input = filter_files_by_country(country_name_input, directory_input)
    if not relevant_files_input:
        print("No files found for the given country.")
        return

    output_filename_input = os.path.join(os.getcwd(), f"{country_name_input.lower()}_data.txt")
    merge_files_into_one(output_filename_input, relevant_files_input, directory_input)

    dates, content = extract_dates_and_content_from_file(output_filename_input)
    ...
    for i, info in zip(dates, content):
        ...
        if len(date)>=3:
            if date[0]=='0':
                date[0]=1
            print(f"{date[0]}-{mydict[date[1]]}-{date[2]}:{info}")
